...
@Service
public class BuscarArmarioPorNomeService {
    ...
    public List<BuscarArmarioResponse> buscar(String nome, Integer pagePost) {
        int armarioPorPagina = 5;

        Pageable pageable = PageRequest.of(pagePost, armarioPorPagina);
        return armarioRepository.findByNome(nome, pageable)
                .stream()
                .map(a -> {
