...
@Service
public class BuscarVendasService {
    ...
    public List<PedidoResponse> get() {
        ...
        return usuario.getVendas().stream().map(venda ->
                pedidoResponseMapper.apply(venda)).filter(PedidoResponse::isAtivo).collect(Collectors.toList());
    }
